...
function M.dump(tbl, maxlines, as_table, _indent, _seen, _output)
  ...
  for k, v in pairs(tbl) do
    ...
    if type(v) == "table" then
      if _seen[v] then
        ...
        M.dump(v, maxlines, as_table, _indent+1, _seen, _output)
      end
  ...
  if _indent==0 then
    if #_output >= maxlines then
      table.insert(_output, string.format("...etc.   -- stopped at %s lines; to show more use table_dump.dump(<table>, [maxlines])", maxlines))
    end
